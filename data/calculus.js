          ...
          "examples": [
            {
              ...
              "solution": "We treat $f$ as a machine and ask which inputs it can legally accept, then which outputs come out.\n\n**Step 1 — Domain.** The square root $\\sqrt{\\;\\cdot\\;}$ only accepts non-negative numbers (over the real numbers). So the expression inside must satisfy\n$$x - 4 \\geq 0.$$\nSolving gives $x \\geq 4$. Thus the domain is\n$$A = \\{\\, x : x \\geq 4 \\,\\} = [4, \\infty).$$\n\n**Step 2 — Range.** For any legal input, $x - 4 \\geq 0$, so $\\sqrt{x-4} \\geq 0$ — every output is at least $0$. Can it equal $0$? Yes: $f(4) = \\sqrt{0} = 0$. Can it get arbitrarily large? Yes: as $x \\to \\infty$, $\\sqrt{x-4} \\to \\infty$. Since $\\sqrt{\\;\\cdot\\;}$ is continuous, every value in between is hit. So the range is\n$$\\{\\, f(x) : x \\in A \\,\\} = [0, \\infty).$$\n\n**Step 3 — Sanity check.** Pick $x = 13$: $f(13) = \\sqrt{13 - 4} = \\sqrt{9} = 3$, which lies in $[0,\\infty)$. Good — exactly one output for that input, as a function requires.\n\n**Answer:** domain $[4, \\infty)$, range $[0, \\infty)$."
            },
            {
              ...
              "solution": "A relation defines $y$ as a function of $x$ exactly when each input $x$ yields **exactly one** output $y$ — equivalently, when every vertical line meets the curve at most once.\n\n**Step 1 — Solve for $y$.** Rearrange:\n$$y^2 = 25 - x^2 \\quad\\Longrightarrow\\quad y = \\pm\\sqrt{25 - x^2}.$$\nThe $\\pm$ is the warning sign: a single $x$ can produce two $y$-values.\n\n**Step 2 — Test a concrete input.** Take $x = 3$:\n$$y = \\pm\\sqrt{25 - 9} = \\pm\\sqrt{16} = \\pm 4.$$\nSo the input $x = 3$ maps to **both** $y = 4$ and $y = -4$. The points $(3, 4)$ and $(3, -4)$ both lie on the circle.\n\n**Step 3 — Vertical line test.** The vertical line $x = 3$ crosses the circle at those two points — two intersections, not one. The test fails.\n\n**Step 4 — Reconcile.** Because the machine would be indecisive (feed it $3$, get back two different answers), $x^2 + y^2 = 25$ is **not** a function of $x$. Note this is a property of the whole relation, not a flaw fixable by choosing nicer numbers: any $-5 < x < 5$ gives two outputs.\n\n**Remark (how to fix it).** Restricting to one branch, $y = +\\sqrt{25 - x^2}$, *is* a function (the upper semicircle); each $x \\in [-5, 5]$ then gives exactly one $y \\geq 0$.\n\n**Answer:** No, $x^2 + y^2 = 25$ does not define $y$ as a function of $x$ — e.g. $x = 3$ gives $y = \\pm 4$, two outputs, failing the vertical line test."
            }
          ]
        },
        {
          ...
          "examples": [
            {
              ...
              "solution": "We differentiate one term at a time. The **sum rule** lets us split the work, and the **constant-multiple rule** lets us pull each coefficient outside the derivative.\n\n**Term 1:** $\\frac{d}{dx}[4x^5] = 4 \\cdot 5x^{4} = 20x^4$ by the power rule $\\frac{d}{dx}[x^n] = nx^{n-1}$.\n\n**Term 2:** $\\frac{d}{dx}[-3x^2] = -3 \\cdot 2x^{1} = -6x$.\n\n**Term 3:** $\\frac{d}{dx}[7x] = 7 \\cdot 1x^{0} = 7$, since $x^0 = 1$.\n\n**Term 4:** $\\frac{d}{dx}[-9] = 0$ by the constant rule — a constant has zero slope.\n\nAdding the pieces together:\n$$f'(x) = 20x^4 - 6x + 7.$$\n\n**Answer:** $f'(x) = 20x^4 - 6x + 7$."
            },
            {
          ...
          "examples": [
            {
              ...
              "solution": "The base here is $2$, not $e$, so we use the general rule $\\frac{d}{dx}a^x = a^x \\ln a$ together with the chain rule.\n\n**Step 1 — Identify the structure.** Write $P(t) = 5 \\cdot a^{u}$ with $a = 2$ and inner function $u = 3t$. The constant $5$ stays out front by the constant-multiple rule.\n\n**Step 2 — Apply the exponential rule with the chain rule.** Differentiating $a^u$ gives $a^u \\ln a \\cdot u'$. Here $u' = \\frac{d}{dt}(3t) = 3$, so\n$$P'(t) = 5 \\cdot 2^{3t} \\ln 2 \\cdot 3 = 15 \\ln 2 \\cdot 2^{3t}.$$\n\n**Step 3 — Sanity check the $\\ln 2$ factor.** Recall that $2^{3t} = e^{3t \\ln 2}$. Differentiating this form with the chain rule gives $e^{3t\\ln 2}\\cdot 3\\ln 2 = 2^{3t}\\cdot 3\\ln 2$ — matching what we got. The $\\ln 2$ is exactly the constant $M(2)$ from the limit definition; it is what makes base $2$ grow slower than base $e$.\n\n**Step 4 — Evaluate at $t = 1$.** Then $2^{3(1)} = 2^3 = 8$, so\n$$P'(1) = 15 \\ln 2 \\cdot 8 = 120 \\ln 2 \\approx 120 (0.6931) \\approx 83.2.$$\n\n**Answer:** $P'(t) = 15\\ln 2 \\cdot 2^{3t}$, and the colony is growing at about $83.2$ thousand bacteria per hour at $t = 1$."
            },
            {
              ...
              "solution": "We treat $\\tan x$ not as a memorized formula but as the quotient $\\frac{\\sin x}{\\cos x}$, then differentiate.\n\n**Step 1 — Set up the quotient rule.** With $f = \\sin x$ and $g = \\cos x$, the quotient rule says $\\left(\\frac{f}{g}\\right)' = \\dfrac{f'g - fg'}{g^2}$. We need the two basic trig derivatives: $f' = \\cos x$ and $g' = -\\sin x$ (note the minus sign on cosine's derivative).\n\n**Step 2 — Substitute.**\n$$\\frac{d}{dx}\\tan x = \\frac{(\\cos x)(\\cos x) - (\\sin x)(-\\sin x)}{\\cos^2 x} = \\frac{\\cos^2 x + \\sin^2 x}{\\cos^2 x}.$$\nThe minus times minus turned the second term positive — a common place to slip.\n\n**Step 3 — Simplify with the Pythagorean identity.** Since $\\cos^2 x + \\sin^2 x = 1$,\n$$\\frac{d}{dx}\\tan x = \\frac{1}{\\cos^2 x} = \\sec^2 x.$$\nThis confirms the standard formula.\n\n**Step 4 — Evaluate the slope at $x = \\frac{\\pi}{4}$.** Here $\\cos\\frac{\\pi}{4} = \\frac{\\sqrt{2}}{2}$, so $\\cos^2\\frac{\\pi}{4} = \\frac{1}{2}$, giving\n$$\\sec^2\\frac{\\pi}{4} = \\frac{1}{1/2} = 2.$$\n\n**Answer:** $\\dfrac{d}{dx}\\tan x = \\sec^2 x$, and the slope of $y = \\tan x$ at $x = \\frac{\\pi}{4}$ is $2$ — the tangent curve is rising at twice the rate of $y = x$ as it passes through the point $\\left(\\frac{\\pi}{4}, 1\\right)$."
            }
          ]
        }
      ]
          ...
          "examples": [
            {
              ...
              "solution": "We treat $y$ as a differentiable function of $x$ and differentiate both sides of $x^2 + y^2 = 1$ with respect to $x$.\n\n**Differentiate term by term.** The term $x^2$ gives $2x$. The term $y^2$ requires the chain rule: differentiating $y^2$ with respect to $x$ gives $2y \\cdot \\frac{dy}{dx}$. The constant $1$ gives $0$:\n$$2x + 2y\\,\\frac{dy}{dx} = 0.$$\n\n**Solve for $\\frac{dy}{dx}$.** Isolate the derivative term and divide:\n$$2y\\,\\frac{dy}{dx} = -2x \\quad\\Longrightarrow\\quad \\frac{dy}{dx} = -\\frac{x}{y}.$$\n\n**Evaluate at the point.** Substitute $x = \\tfrac{1}{2}$ and $y = \\tfrac{\\sqrt{3}}{2}$:\n$$\\frac{dy}{dx} = -\\frac{\\tfrac{1}{2}}{\\tfrac{\\sqrt{3}}{2}} = -\\frac{1}{\\sqrt{3}} = -\\frac{\\sqrt{3}}{3}.$$\n\n**Sanity check.** On a circle the radius to a point is perpendicular to the tangent there. The radius has slope $\\frac{\\sqrt{3}/2}{1/2} = \\sqrt{3}$, and the tangent slope $-\\frac{1}{\\sqrt{3}}$ is its negative reciprocal — exactly as expected.\n\n$$\\boxed{\\dfrac{dy}{dx} = -\\dfrac{x}{y}, \\qquad \\left.\\dfrac{dy}{dx}\\right|_{\\left(\\frac{1}{2},\\,\\frac{\\sqrt{3}}{2}\\right)} = -\\dfrac{\\sqrt{3}}{3}.}$$"
            },
            {
              ...
              "solution": "Let $x$ be the distance from the wall to the base of the ladder and $y$ the height of the top of the ladder, both functions of time $t$. The ladder length is fixed, so the Pythagorean relation holds at all times:\n$$x^2 + y^2 = 10^2 = 100.$$\n\n**Differentiate both sides with respect to time.** Here every variable is an unspecified function of $t$, so the chain rule attaches a rate to each term:\n$$2x\\,\\frac{dx}{dt} + 2y\\,\\frac{dy}{dt} = 0.$$\nDividing by $2$:\n$$x\\,\\frac{dx}{dt} + y\\,\\frac{dy}{dt} = 0.$$\n\n**Find the missing geometric value.** At the instant in question $x = 6$. From $x^2 + y^2 = 100$:\n$$y = \\sqrt{100 - 36} = \\sqrt{64} = 8 \\text{ ft}.$$\n\n**Insert the known rate and solve for $\\frac{dy}{dt}$.** The base moves away from the wall, so $\\frac{dx}{dt} = +2$ ft/s. Substitute $x = 6$, $y = 8$:\n$$6(2) + 8\\,\\frac{dy}{dt} = 0 \\quad\\Longrightarrow\\quad 12 + 8\\,\\frac{dy}{dt} = 0.$$\nSolving:\n$$\\frac{dy}{dt} = -\\frac{12}{8} = -\\frac{3}{2} = -1.5 \\text{ ft/s}.$$\n\n**Interpret the sign.** The negative value means $y$ is decreasing: the top slides *down* the wall, which matches the physical picture.\n\n$$\\boxed{\\dfrac{dy}{dt} = -1.5 \\text{ ft/s} \\;\\;(\\text{the top descends at } 1.5 \\text{ ft/s}).}$$"
            }
          ]
        },
        {
          ...
          "examples": [
            {
              ...
              "solution": "**Step 1 — Differentiate and find critical points.** Since $f$ is a polynomial, $f'$ exists everywhere, so the only critical points come from $f'(x) = 0$.\n$$f'(x) = 3x^2 - 12x + 9 = 3(x^2 - 4x + 3) = 3(x-1)(x-3).$$\nSetting $f'(x) = 0$ gives $x = 1$ and $x = 3$.\n\n**Step 2 — Sign chart for $f'$.** The roots $x=1$ and $x=3$ split the line into three intervals. Test one point in each:\n- $x = 0$: $f'(0) = 3(-1)(-3) = 9 > 0$, so $f$ is **increasing** on $(-\\infty, 1)$.\n- $x = 2$: $f'(2) = 3(1)(-1) = -3 < 0$, so $f$ is **decreasing** on $(1, 3)$.\n- $x = 4$: $f'(4) = 3(3)(1) = 9 > 0$, so $f$ is **increasing** on $(3, \\infty)$.\n\n**Step 3 — Apply the First Derivative Test.** At each critical point, read how the sign of $f'$ changes:\n- At $x = 1$: $f'$ goes $+ \\to -$ (increasing then decreasing), so $f$ has a **local maximum**.\n- At $x = 3$: $f'$ goes $- \\to +$ (decreasing then increasing), so $f$ has a **local minimum**.\n\n**Step 4 — Compute the extreme values.**\n$$f(1) = 1 - 6 + 9 + 1 = 5, \\qquad f(3) = 27 - 54 + 27 + 1 = 1.$$\n\n**Answer.** $f$ increases on $(-\\infty,1)$, decreases on $(1,3)$, increases on $(3,\\infty)$. There is a local maximum of $5$ at $x = 1$ and a local minimum of $1$ at $x = 3$."
            },
            {
              ...
              "solution": "**Step 1 — First derivative and critical points.**\n$$f'(x) = 4x^3 - 12x^2 = 4x^2(x - 3).$$\nSetting $f'(x) = 0$ gives $x = 0$ (a double root) and $x = 3$.\n\n**Step 2 — Second derivative.**\n$$f''(x) = 12x^2 - 24x = 12x(x - 2).$$\n\n**Step 3 — Second Derivative Test at each critical point.**\n- At $x = 3$: $f''(3) = 12(3)(1) = 36 > 0$. The curve is concave up, so $f$ has a **local minimum** at $x = 3$, with $f(3) = 81 - 108 = -27$.\n- At $x = 0$: $f''(0) = 12(0)(-2) = 0$. The test is **inconclusive** — we must fall back on the First Derivative Test. Near $x = 0$, $f'(x) = 4x^2(x-3)$. The factor $4x^2 \\ge 0$ always, and for $x$ near $0$ the factor $(x-3) < 0$, so $f'(x) \\le 0$ on both sides of $0$. Since $f'$ does **not** change sign at $x=0$, this critical point is **neither a max nor a min** (the curve flattens but keeps decreasing).\n\n**Step 4 — Concavity and inflection points.** Set $f''(x) = 12x(x-2) = 0$, giving candidate inflection points at $x = 0$ and $x = 2$. Build a sign chart for $f''$:\n- $x = -1$: $f''(-1) = 12(-1)(-3) = 36 > 0 \\Rightarrow$ concave **up** on $(-\\infty, 0)$.\n- $x = 1$: $f''(1) = 12(1)(-1) = -12 < 0 \\Rightarrow$ concave **down** on $(0, 2)$.\n- $x = 3$: $f''(3) = 12(3)(1) = 36 > 0 \\Rightarrow$ concave **up** on $(2, \\infty)$.\n\nConcavity changes at both $x = 0$ and $x = 2$, so both are genuine inflection points:\n$$f(0) = 0, \\qquad f(2) = 16 - 32 = -16.$$\n\n**Answer.** Local minimum $-27$ at $x = 3$; the critical point at $x = 0$ is neither a max nor a min (here the Second Derivative Test fails and the First Derivative Test settles it). $f$ is concave up on $(-\\infty,0)$ and $(2,\\infty)$, concave down on $(0,2)$, with inflection points at $(0, 0)$ and $(2, -16)$."
            }
          ]
        },
        {
          ...
          "examples": [
            {
              ...
              "solution": "Let $f(x) = \\sqrt[3]{x} = x^{1/3}$ and choose the nearby easy point $a = 8$, since $\\sqrt[3]{8} = 2$ exactly.\n\n**Step 1 — Evaluate $f$ and $f'$ at $a$.** We have $f(a) = f(8) = 8^{1/3} = 2$. The derivative is $f'(x) = \\frac{1}{3}x^{-2/3} = \\frac{1}{3\\sqrt[3]{x^2}}$, so\n$$f'(8) = \\frac{1}{3\\sqrt[3]{8^2}} = \\frac{1}{3\\cdot 4} = \\frac{1}{12}.$$\n\n**Step 2 — Write the linearization.** $$L(x) = f(a) + f'(a)(x-a) = 2 + \\tfrac{1}{12}(x - 8).$$\n\n**Step 3 — Plug in $x = 8.12$.** Here $x - a = 0.12$, so\n$$\\sqrt[3]{8.12} \\approx L(8.12) = 2 + \\frac{1}{12}(0.12) = 2 + 0.01 = 2.01.$$\n\n**Step 4 — Check the quality.** The true value is $\\sqrt[3]{8.12} \\approx 2.00995$, so the error is about $5 \\times 10^{-5}$ — excellent, because $0.12$ is small relative to $a = 8$ and the curve is nearly straight there.\n\n**Answer:** $\\sqrt[3]{8.12} \\approx 2.01$."
            },
            {
        ...
        },
        {
          ...
          "examples": [
            ...
            },
            {
              ...
              "solution": "**Step 1 — Compute the gradient.**\n$$\\frac{\\partial g}{\\partial x} = 3x^2 - 3y, \\qquad \\frac{\\partial g}{\\partial y} = -3x + 3y^2.$$\n\n**Step 2 — Solve $\\nabla g = \\mathbf{0}$.** Setting both partials to zero and dividing by $3$:\n$$x^2 - y = 0 \\;\\Rightarrow\\; y = x^2, \\qquad -x + y^2 = 0 \\;\\Rightarrow\\; x = y^2.$$\nSubstitute $y = x^2$ into $x = y^2$: $x = (x^2)^2 = x^4$, so $x^4 - x = 0$, i.e. $x(x^3 - 1) = 0$. Over the reals this gives $x = 0$ or $x = 1$.\n- If $x = 0$: $y = x^2 = 0 \\Rightarrow (0,0)$.\n- If $x = 1$: $y = x^2 = 1 \\Rightarrow (1,1)$.\n\nThe other roots of $x^3 = 1$ are complex, so we discard them. The real critical points are $(0,0)$ and $(1,1)$.\n\n**Step 3 — Form the Hessian.** Here the Hessian depends on the point:\n$$H(x,y) = \\begin{bmatrix} g_{xx} & g_{xy} \\\\ g_{yx} & g_{yy} \\end{bmatrix} = \\begin{bmatrix} 6x & -3 \\\\ -3 & 6y \\end{bmatrix}, \\qquad \\det H = 36xy - 9.$$\n\n**Step 4 — Classify $(0,0)$.**\n$$\\det H(0,0) = 36(0)(0) - 9 = -9 < 0.$$\nA negative determinant means the Hessian has one positive and one negative eigenvalue: the surface curves up in one direction and down in another. So $(0,0)$ is a **saddle point** — the distinctively multivariable creature with no single-variable analogue.\n\n**Step 5 — Classify $(1,1)$.**\n$$\\det H(1,1) = 36(1)(1) - 9 = 27 > 0.$$\nThe extremum is genuine; check the top-left entry $g_{xx} = 6(1) = 6 > 0$, so $H$ is positive definite and the surface curves upward in all directions. Thus $(1,1)$ is a **local minimum**, with value $g(1,1) = 1 - 3 + 1 = -1$.\n\n**Step 6 — A word on the inconclusive case.** Notice the test would stall only where $\\det H = 0$ (e.g. if a critical point landed on $xy = \\tfrac14$); there the second-derivative test gives no verdict and you must inspect $g$ directly. Neither of our points hits that wall, so both are cleanly classified.\n\n**Answer:** $(0,0)$ is a saddle point; $(1,1)$ is a local minimum with $g(1,1) = -1$."
            }
          ]
        }
      ]
