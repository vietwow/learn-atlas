...
2. **Deepen the existing six** — ✅✅ COMPLETE (iter 99). Every original topic now has a 7th frontier/advanced module,
   ...
   ⏳ MODULE 6 IN PROGRESS (deepening PS per owner direction + the doc'd "optional future"). Remaining: top ps-bayesian-inference 8→16 MCQ; add lessons — (2) Conjugate priors in depth (Beta–Binomial, Gaussian–Gaussian, Gamma–Poisson), (3) From posterior to decisions (MAP, credible intervals, posterior predictive), (4) Computational Bayes (MCMC & variational inference, intuition). Maybe a Beta-distribution viz (new build).
